...
conf.register( 'watch', iai.project.resolve('backend'), conf.types.path );
...
function boot(){
  ...
  child.on( 'exit', function( code, signal ){
    ...
    return log( 'wait for a change to reboot' );
  ...
  log( 'watching for changes on %s', iai.conf.watch );
  watch( iai.conf.watch, {
    persistent: false,
    ignoreInitial: true
  })
