 ...
 * @pattern Proxy
 *   @param builder: A function that creates Error instances
 *
...
module.exports = oop.builder(function( builder ){
  ...
  toString: function(){
    return !this.length
      ? this.name + ' (empty)'
      : [ this.name,
          this.map(function(e){ return '  '+e.message; }).join('\n'),
          ''
        ].join('\n')
    ;
  }
