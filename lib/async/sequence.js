...
function sequence(iterable, step, complete, context) {
  if( !isFn(step) || !isFn(complete) ){
    throw TypeError("must provide step and complete callback as functions");
  }
  var sequence = [], context = context || null;
  // push steps on sequence
  for( var key in iterable ) {
    sequence.push( step.bind( context, key, iterable[key], function next(err){
      ...
      if( err && err instanceof Error ){
        ...
        throw TypeError( "sequence#next received non-error as error" );
      }
